...
const ForgotPassword = () => {
  ...
  setServerMessage("");
  try {
    const res = await forgotPassword(data.email, captchaToken);
    setServerMessage(res.message || "Password reset link sent to your email.");
  } catch (err: any) {
    setServerMessage(err.response?.data?.error || "Something went wrong");
  }
};
