...
const ResetPassword = () => {
  ...
  try {
    ...
    setServerMessage(err.response?.data?.error || "Invalid or expired token");
  }
};
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {(Object.values(errors).length > 0 || serverMessage) && (
            <div className="text-center mt-2 space-y-1">
              ...
              {serverMessage && (
                <p className={`text-sm ${serverMessage.toLowerCase().includes("success") ? "text-green-500" : "text-red-500"}`}>
                  {serverMessage}
                </p>
              )}
            </div>
