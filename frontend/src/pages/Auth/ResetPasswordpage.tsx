...
function getErrorMessages(errors: unknown): string[] {
  ...
  if (typeof errors !== "object" || errors === null) return messages;
  ...
  Object.values(errors).forEach((err) => {
    ...
    if (typeof err === "object" && "message" in err && typeof (err ).message === "string") {
      ...
      messages.push(...getErrorMessages(err));
    }
  });
...
const ResetPassword = () => {
  ...
  const onSubmit = async (data: ResetPasswordFormData) => {
    if (!token) {
      setServerMessage("Invalid or expired reset link.");
      return;
    }
    ...
    try {
      ...
      setServerMessage(res.message || "Password reset successfully. You can login now.");
      setTimeout(() => navigate("/login"), 3000);
    } catch (err: unknown) {
      const axiosErr = err as ApiError;
      setServerMessage(axiosErr.response?.data?.error || "Invalid or expired token");
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
      ...
      <div className="max-w-md w-full bg-white p-8 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {(Object.keys(errors).length > 0 || serverMessage) && (
